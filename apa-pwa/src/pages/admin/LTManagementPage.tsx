...
const LTManagementPage: React.FC = () => {
    const [leads, setLeads] = useState<any[]>([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const q = query(collection(db, 'leads_lt'), orderBy('createdAt', 'desc'));
    ...
    const handleDelete = async (id: string, name: string) => {
        if (confirm(`Excluir o registro de Lar Temporário de ${name}?`)) {
            try {
                await deleteDoc(doc(db, 'leads_lt', id));
            } catch (error) {
                console.error(error);
            }
        }
    ...
    const updateStatus = async (id: string, newStatus: string) => {
        try {
            await updateDoc(doc(db, 'leads_lt', id), {
                status: newStatus,
    ...
    const exportToCSV = () => {
        ...
        const headers = ['Data', 'Nome', 'Email', 'Telefone', 'Endereço', 'Status', 'Motivo Rejeição'];
        ...
        link.setAttribute("download", `lares_temporarios_${new Date().toISOString().split('T')[0]}.csv`);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };

    if (loading) return <div className="p-20 text-center animate-pulse">Carregando candidatos...</div>;
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <h2 className="text-2xl font-bold text-gray-800 font-merriweather">Candidatos a Lar Temporário</h2>
                </div>
                            ...
                            <div className="space-y-4 flex-grow">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex gap-2">
                                        {l.status === 'pending' && <Badge variant="info" className="uppercase text-[9px]">Pendente</Badge>}
                                        {l.status === 'approved' && <Badge variant="success" className="uppercase text-[9px]">Aprovado</Badge>}
                                        {l.status === 'rejected' && <Badge variant="warning" className="uppercase text-[10px]">Reprovado</Badge>}
                                        {l.status === 'contacted' && <Badge variant="success" className="uppercase text-[9px]">Contatado</Badge>}
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                                    ...
                                    <div className="flex items-center gap-2 md:col-span-2">
                                        <MapPin size={16} className="text-gray-400" /> {l.address}
                                    </div>
                                </div>

                                <div className="flex flex-wrap gap-2">
                                    <Badge variant="outline" className="gap-1.5 py-1.5 px-3 border-gray-100">
                                        <Users size={12} /> {l.householdCount} moradores
                                    </Badge>
                                    <Badge variant="outline" className="gap-1.5 py-1.5 px-3 border-gray-100">
                                        <Home size={12} /> {l.dwellingType}
                                    </Badge>
                                    <Badge variant="outline" className={`gap-1.5 py-1.5 px-3 border-gray-100 ${l.hasOtherPets === 'sim' ? 'text-brand-orange' : 'text-gray-400'}`}>
                                        <Dog size={12} /> Tem outros pets: {l.hasOtherPets}
                                    </Badge>
                                </div>
                                ...
                                {l.spaceDescription && (
                                    <div className="p-4 bg-white rounded-2xl border border-gray-100 relative">
                                        <Info size={14} className="absolute -top-2 -left-2 text-brand-orange" />
                                        <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">Espaço & Disponibilidade</p>
                                        <p className="text-sm text-gray-600 italic">"{l.spaceDescription}"</p>
                                        <p className="text-xs text-brand-green font-bold mt-2 flex items-center gap-1">
                                            <Clock size={12} /> {l.availability}
                                        </p>
                                    </div>
                                )}
                            </div>
            ...
            {leads.length === 0 && (
                <div className="text-center p-20 border-2 border-dashed border-gray-100 rounded-3xl">
                    <p className="text-gray-400 font-medium">Nenhum candidato a Lar Temporário registrado ainda.</p>
                </div>
            )}
        </div>
    );
...
export default LTManagementPage;
